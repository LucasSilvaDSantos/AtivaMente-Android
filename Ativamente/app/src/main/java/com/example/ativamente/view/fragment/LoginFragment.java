...
public class LoginFragment extends Fragment {
    ...
    public void onStart() {
        super.onStart();
        // Verifica se o usuário já está logado (seja conta real ou anônima) e vai para a Home
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        // Clique do botão de Login tradicional
        binding.buttonLogin.setOnClickListener(v -> loginUser());

        // Clique do botão "Continuar como Convidado" (Adicionado aqui)
    ...
    // Método para realizar o Login Anônimo no Firebase (Adicionado aqui)
    private void loginAnonymously() {
        binding.btnGuestLogin.setEnabled(false); // Desabilita para evitar múltiplos cliques seguidos

        mAuth.signInAnonymously()
                .addOnCompleteListener(task -> {
                    binding.btnGuestLogin.setEnabled(true); // Reativa o botão
                    if (task.isSuccessful()) {
                        Toast.makeText(getContext(), "Entrando como convidado...", Toast.LENGTH_SHORT).show();
                        goToHome(); // Navega direto para a Home usando a sua estrutura
                    } else {
